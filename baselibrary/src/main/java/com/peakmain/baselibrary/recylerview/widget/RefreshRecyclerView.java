...
public class RefreshRecyclerView extends WrapRecyclerView {
    ...
    private void restoreRefreshView() {
        ...
        animator.addUpdateListener(animation -> {
            float currentTopMargin1 = (float) animation.getAnimatedValue();
            setRefreshViewMarginTop((int) currentTopMargin1);
        });
        animator.start();
        mCurrentDrag = false;
    }
    ...
    private void updateRefreshStatus(int distanceY) {
        ...
        }else if (distanceY < mRefreshViewHeight) {//小于头部布局的高度
    ...
    private void addRefreshView() {
        ...
        if (adapter != null && mRefreshCreator != null) {
            //添加头部刷新的view
            View view = mRefreshCreator.getRefreshView(getContext(), this);
            if (view != null) {
                addHeaderView(view);
